 ...
 * Keep in sync with `src/lib/taskPermissionAliases.ts`.
 */
...
/** Catalog `tasks.*` or a PRD §5.1 alias string (used with `tasks.manage` umbrella). */
export function isTaskRbacActionName(required: string): boolean {
    if (required.startsWith('tasks.')) return true;
    for (const g of TASK_PERMISSION_EQUIVALENCE_GROUPS) {
