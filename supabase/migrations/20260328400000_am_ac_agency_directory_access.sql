-- Allow Account Managers and Account Coordinators to call tenant_directory_for_admin
-- on their OWN agency tenant, so the AgencyConsoleView can display full member names/emails.
-- Previously this RPC only permitted AMs on *seller* tenants (from migration 20260328200000).
...
COMMENT ON FUNCTION public.tenant_directory_for_admin IS
    'List members of a tenant. Allowed for: platform ops, Agency Admin, Agency AM/AC members, Seller Admin, parent Agency Admin, or AM assigned to this seller.';
