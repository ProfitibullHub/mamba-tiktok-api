-- Platform Super Admins (and legacy profiles.role = 'admin') use a platform tenant context where
-- RBAC view_cogs / view_margin are not granted on the platform tenant. Without a baseline grant they
-- incorrectly receive can_view_* = false for every seller P&L while seller_financial_visibility_rules
-- never target them (principal resolution is agency/seller-only).
-- Fix: grant full baseline visibility for platform operators; seller_financial_visibility_rules still
-- apply when a rule explicitly targets their resolved principal (future) or agency/seller principals.
