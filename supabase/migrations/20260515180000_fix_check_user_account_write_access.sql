-- Restore account-tenant scoping for write checks (20260416120000 incorrectly joined memberships on profiles.tenant_id).
...
COMMENT ON FUNCTION public.check_user_account_write_access(uuid, uuid) IS
    'True if p_user_id may mutate data for p_account_id. Denies Seller User on the account''s seller tenant; allows Seller Admin and agency/platform roles.';
