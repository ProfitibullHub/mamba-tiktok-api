-- Re-invite after decline: ON CONFLICT must reset status declined → invited so role assignment
-- clears the prior rejection; invite-member API must also treat declined as needing a new token (server change).
...
BEGIN
    ...
    SET role_id = EXCLUDED.role_id,
        status = CASE
            WHEN tenant_memberships.status = 'declined' THEN 'invited'
